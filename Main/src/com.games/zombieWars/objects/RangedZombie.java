...
public class RangedZombie extends GameObject {
    ...
    private final int delay = genDelay();

    private int genDelay() {
        int delay = new Random().nextInt(121);
        if (delay < 50) {
            return genDelay();
        }
        return delay;
    ...
    public RangedZombie(float x, float y, ID id, float speedX, float speedY, int difficulty, int hp, int width, int height, Handler handler) {
    ...
    private void tryShoot() {
        ...
        assert player != null;
        LinkedList<LinkedList<Integer>> path = findLine((int) getX(), (int) getY(), (int) player.getX(), (int) player.getY());
        for (GameObject o : obstacleList) {
            for (int i = 0; i < path.size(); i++) {
                if ((new Rectangle((int) o.getX(), (int) o.getY(), o.getWidth(), o.getHeight())).contains(new Point(path.get(i).get(0), path.get(i).get(1)))) {
                    return;
                }
            }
        }
    ...
    private void zombieShoot() {
        if (shootDelay == 0) {
            GameObject gameObject = handler.addObject(new ZombieBullet(
                    getX(),
                    getY(),
                    ID.Bullet,
                    0,
            ...
            gameObject.setVelX((float) (((-1.0 / distance) * diffX) * 3) * HUD.bulletSpeed);
            gameObject.setVelY((float) (((-1.0 / distance) * diffY) * 3) * HUD.bulletSpeed);
            shootDelay++;
        } else {
            shootDelay++;
            if (shootDelay == delay) {
